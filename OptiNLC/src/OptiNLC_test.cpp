...
main()
{
  ...
  if( !settings )
  {
    std::cerr << "Failed to allocate memory for OSQP settings." << std::endl;
    return -1;
  }
  ...
  if( exitflag != 0 )
  {
    std::cerr << "OSQP setup failed with exit code: " << exitflag << std::endl;
  ...
  {
    std::cerr << "OSQP solve failed with exit code: " << exitflag << std::endl;
  }
